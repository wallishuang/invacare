...
public class PolicyholderService {
	 ...
	 * @throws Exception
	 */
	public String addHolder(PolicyholderCreatedVO createVO) throws Exception {
	 ...
	 * @throws Exception
	 */
	public List<Policyholder> getHolderByView(String code) throws Exception {
		List<Policyholder> result = new ArrayList<>();
		
		// 依編號取得保戶資訊
		Policyholder entity = null;
		if(StringUtils.isNotBlank(code)) {
			entity = repository.findByCode(code);			
		} else {
			entity = repository.findRootCode();	
		}
		if (entity == null) {
			return result;
		}
		...
		// 依編號取得該保戶下的子節點
		List<Policyholder> subEntities = repository.findByParentCodeOrderByParentPosition(entity.getCode());
		if (CollectionUtils.isEmpty(subEntities)) {
			return result;
		}
		if (!CollectionUtils.isEmpty(subEntities)) {
			// 遞迴尋找子節點是否能繼續往下，直到子節點沒有節點
			for (Policyholder holder : subEntities) {
				result.addAll(getHolderByView(holder.getCode()));
			}
		}
